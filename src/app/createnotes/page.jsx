...
function Createnote() {
  ...
  const handleSubmit = async (values, { resetForm }) => {
    ...
    try {
      ...
      alert("Note created successfully!, Go to My Notes to view it.")
    } catch (error) {
  ...
  return (
    ...
    <div className="min-h-screen flex bg-gray-50">
      ...
      {display && (
        <div className="fixed inset-0  bg-opacity-90 flex items-center justify-center z-50 min-h-screen" onClick={() => setDisplay(false)}>
            ...
            <Formik initialValues={iv} validationSchema={formValidation} onSubmit={handleSubmit}>
              <Form className='space-y-5'>
                <div className="flex flex-col gap-2">
                  <Field name="title" placeholder="Note Title" className="outline-none border rounded-md border-gray-200 p-2 w-full text-gray-800" />
                  <ErrorMessage name='title' component={"p"} className='text-xs text-red-600' />
                </div>
                <div className="flex flex-col gap-2">
                  <Field name="note" as="textarea" placeholder="Note Content" className="outline-none border rounded-md border-gray-200 p-2 w-full text-gray-800" />
